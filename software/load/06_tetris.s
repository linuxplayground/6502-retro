...
; empty bag_of_7 table if it's full.
; Find a random number from 0 - 7
; look up in bag_of_7 table.  Set if empty
; select again if unavailable.
get_random:
        ldx #7
:       lda bag_of_7,x
        beq :++                 ; we found an empty slot jump to get random
        dex
        bne :-
        ldx #7                  ; no empty slots found, empty the bag_of_7
:       stz bag_of_7,x
        dex
        bne :-
:       jsr prng                ; get random
...
:       inc a
        tax
        lda bag_of_7,x
        bne :--                 ; if the bag_of_7 at random loc is not zero, try again
        txa
        sta bag_of_7,x          ; mark that piece as selected
        txa                     ; result in A
        dec a
:       rts
